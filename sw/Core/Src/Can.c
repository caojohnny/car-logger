...
HAL_StatusTypeDef canTransmit(CAN_HandleTypeDef* canHandle, enum CanId target, const uint8_t* data, uint16_t dataLength)
{
    ...
    uint32_t mb;
    return HAL_CAN_AddTxMessage(canHandle, &header, data, &mb) != HAL_OK;
}

static void canReceiveIsr(CAN_HandleTypeDef* canHandle)
{
...
HAL_StatusTypeDef canInit(CAN_HandleTypeDef* canHandle)
{
    ...
    status = HAL_CAN_RegisterCallback(canHandle, HAL_CAN_RX_FIFO0_MSG_PENDING_CB_ID, canReceiveIsr);
    if (status != HAL_OK) {
        return status;
    }
