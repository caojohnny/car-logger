...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      uint8_t dataLength = packObd2Data(packed, &obd2Data, ISO_TP_SF_SIZE);

      HAL_StatusTypeDef tx_result = canTransmit(&hcan, CAN_ID_FUNC_ADDR, packed, dataLength);
      if (tx_result != HAL_OK)
      {
          Error_Handler();
      }
